...
| **Smoke test** | `bash scripts/smoke.sh` — 22 curl-based checks across every API group plus deterministic write-paths (stock-take open → count → cancel, RTV over-return guard 400, expense-digest shape). Zero test framework, exit-code driven; run before every push. |
