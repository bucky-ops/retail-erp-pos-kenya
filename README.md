...
| **Supplier statements** | Procurement › **Suppliers › Statement**: a printable A4 reconciliation sheet per supplier - goods **received vs ordered**, **RTV debit notes** (credited vs pending, with a "follow up" alert when credits are outstanding), the **negotiated price list** with per-unit savings vs catalog, per-PO received/outstanding columns and a **net traded** position (`/api/suppliers/[id]/statement`). Deliverable three ways: **Print / Save PDF**, **Email statement** (full reconciliation preview → send, saved on the supplier for next time, audited as `Email/Supplier Statement` in Messages - `/api/suppliers/[id]/statement/email`). |
...
| **PO basket preview** | Procurement › **Reorder suggestions › Preview basket**: a review drawer showing **exactly what "Create purchase orders" will commit** - lines grouped per supplier (name, lead time, subtotal) then per store (one PO each, per-store subtotals, every line as `qty × negotiated cost = line total`), plus a green basket-total banner and a **Confirm & create N POs** CTA. No more blind commits - buyers see the split before it lands in the Purchase orders tab. |
| **Receivables aging chart** | Debts › **Receivables aging**: a live stacked bar + four clickable bucket cards (Current / 1-30 / 31-60 / 60+ days) computed from the active ledger - each card shows amount, ledger count and share %, and **clicking a bucket filters the debtor list below** (click again to clear, mirrored by a "Filtered" chip in the panel header). Credit-control triage without leaving the screen. |
...
| **Smoke test** | `bash scripts/smoke.sh` - 25 curl-based checks across every API group plus deterministic write-paths (stock-take open → count → cancel, RTV over-return guard 400, expense-digest shape, supplier statement reconciliation, statement email previews for debtors AND suppliers). Zero test framework, exit-code driven; run before every push. |
